                ...
                <div>
                    <div class=" table-responsive table-desi">
                        <table class="email table table-striped">
                            ...
                            <tbody>
                                @foreach (array_reverse($data) as $key=>$email)
                                    @if ($key >= (($currentPage*$messagesPerPage)-$messagesPerPage) && $key <($currentPage*$messagesPerPage))
                                        @if (trim($email->Unseen) == 'U')
                                            <tr class="unseen">
                                                <td>{{ $email->subject }}</td>
                                                <td>{{ $email->fromaddress }}</td>
                                                <td>{{ $email->date }}</td>
                                                <td>
                                                <a href="{{ url('view-email/'.count($data)-$key)}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                                </td>
                                                {{--
                                                <td>
                                                <a href="{{ url('delete-mails/'.$item->id)}}" class="btn btn-danger"><i class="fa-solid fa-trash" style="color: #d51010;"></i></a>
                                                </td> --}}
                                            </tr>
                                        @else
                                            <tr class="">
                                                ...
                                                <td>{{ $email->date }}</td>
                                                <td>
                                                <a href="{{ url('view-email/'.count($data)-$key)}}" class="btn btn-light"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                                </td>
                                                {{--
                                                <td>
                                                <a href="{{ url('delete-mails/'.$item->id)}}" class="btn btn-danger"><i class="fa-solid fa-trash" style="color: #d51010;"></i></a>
