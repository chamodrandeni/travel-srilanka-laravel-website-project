...
@section('title', 'Update Tour Plane Image')
...
<div class="row">
    <div class="col-12">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <form class="theme-form mega-form" action="{{ url('update-tourplanimage/'.$tourplanimage->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-header">
                            <h5>Update Tour Plan Image's</h5>
                        </div>
                        <div class="card-body">
                            <div class="mb-3">
                                <label class="form-label-title">Tour Plan</label>
                                <select class="form-control js-example-basic-single col-sm-12" name="tourplan_id">
                                    <option value="">Select Tour Plan</option>
                                    @foreach ($tourplan as $item)
                                        <option value="{{ $item->id }}" {{ $tourplanimage->tourplan_id ? 'selected' : '' }}>{{ $item->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @if ($tourplanimage->image)
                            <img src="{{ asset('assets/uploads/tourplanimage/'.$tourplanimage->image) }}" alt="Category Image" class="imgedit">
                            @endif
                            <div class="mb-3">
                                <label class="form-label-title">Images</label>
                                <input type="file" class="form-control" placeholder="Update tour Plan Images" name="image">
                            </div>
                        </div>
