...
@section('title', 'Update Tour Plan')
...
<div class="row">
    <div class="col-12">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <form class="theme-form mega-form" action="{{ url('update-tourplan/'.$tour_plan->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-header">
                            <h5>Update Tour Plan</h5>
                        </div>
                        <div class="card-body">
                            <div class="mb-3">
                                <label class="form-label-title">Tour Plan Title</label>
                                <input class="form-control" type="text" value="{{ $tour_plan->title }}" placeholder="Enter Package Name" name="title">
                            </div>
                            <div class="mb-3">
                                <label class="form-label-title">Package</label>
                                <select class="form-control js-example-basic-single col-sm-12" name="packages_id">
                                    <option value="">Select Package</option>
                                    @foreach ($packages as $item)
                                        <option value="{{ $item->id }}" {{ $tour_plan->packages_id ? 'selected' : '' }}>{{ $item->packname }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label-title  ">Tour Plan Description</label>
                                <textarea name="description" rows="3" placeholder="Enter Package Description" class="form-control">{{ $tour_plan->description }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label  class="form-label-title ">Status</label>
                                <select class="form-control js-example-basic-single col-sm-12" name="status">
                                    <option value="">Select Status</option>
                                    <option value="1" {{ $tour_plan->status == 1 ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ $tour_plan->status == 0 ? 'selected' : '' }}>Deactive</option>
                                </select>
                            </div>
                        </div>
