...
<div class="row">
    <div class="col-12">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <form class="theme-form mega-form" action="{{ url('update-gallery/'.$gallery->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input class="form-control" type="text" value="{{ $gallery->title }}" placeholder="Category name" name="title">
                            </div>
                            <div class="mb-3">
                                <label class="form-label-title">Category</label>
                                <select class="form-control js-example-basic-single col-sm-12" name="category">
                                    <option value="">Select Category</option>
                                    @foreach ($imagecategory as $item)
                                        <option value="{{ $item->id }}" {{ $gallery->category ? 'selected' : '' }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                            ...
                            @if ($gallery->gallary_image)
                            <img src="{{ asset('assets/uploads/gallery/'.$gallery->image) }}" alt="Category Image" class="imgedit">
                            @endif
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label class="form-label-title">Meta Keywords</label>
                                <textarea name="meta_keywords" rows="3" class="form-control">{{ $gallery->meta_keywords }}</textarea>
                            </div>
                        </div>
